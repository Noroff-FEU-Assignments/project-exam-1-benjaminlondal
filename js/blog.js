...
async function getPosts() {
  ...
  pageNr++;
  const blogPostsUrl =
    "https://blog-api-posts.dvergnir.one/wp-json/wp/v2/posts?page=" +
    pageNr +
    "&_embed";
...
function postTemplate(postData) {
  ...
  const blogImageAlt =
    postData._embedded["wp:featuredmedia"]?.[0].alt_text ?? "Missing alt text";
...
function filterPosts(searchValue, postsList) {
  ...
  const filteredPosts = postsList.filter(function (postData) {
    if (
      postData.title.rendered.toLowerCase().includes(searchValue) ||
      postData.excerpt.rendered.toLowerCase().includes(searchValue)
    ) {
      return true;
    }
  });
